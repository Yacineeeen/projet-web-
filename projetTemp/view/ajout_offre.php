...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['stripeToken'])) {
    ...
    \Stripe\Stripe::setApiKey('sk_test_51OJMBwAZmmEr24Ck9oLM10KmhHqxuBg5edBXhRdX9MAtQ8R5xKm2w3NWYBRFzMKnczoQEAvCGAtLCsz1eRKcFEqp00eyaQxtP6'); // Remplacez par votre clé secrète Stripe

    // Récupérer le token de la requête POST
    $token = isset($_POST['stripeToken']) ? $_POST['stripeToken'] : '';

    if (empty($token)) {
        // Si le token est vide, ne faites rien ou affichez un message approprié
    } else {
        try {
            $charge = \Stripe\Charge::create([
                'amount' => 1000, // Remplacez par le montant réel
                'currency' => 'usd',
                'description' => 'Paiement pour votre produit ou service',
                'source' => $token,
            ]);
